...
// char의 reference를 return 하는 이유는 이 클래스는 실제로 exam[10] = 'c' 와 같은 역할을 하기 떄문이다.
// reference를 이용해 존재하는 string의 space에 있는 char을 가르키도록 하는 것이다.
...
/*
bug : 사용자가 실수로 문자열의 실제 길이 보다 더 많이 지울 때 에러
1. 사용자에게 error message를 print 하고 다시 값을 입력 받는 방법
2. 만약 "abcdef" 문자열에서 사용자가 실수로 erase(4, 3) 을 입력 했다면, 
사용자가 4의 자리인 'e'부터 나머지 뒷 부분을 다 지우고 싶어한다고 이해하고, string_length 를 4로 주어 d의 뒷부분은 다 처리하는 것
 */ 
